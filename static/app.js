...
function quickFilterChipFieldLabel(view, field) {
  ...
  if (field === "liquidity") return "Liquidity";
...
function quickFilterFieldLabel(view, field) {
  ...
  if (view === "accounts" && field === "liquidity") return "Liquidity";
...
function quickFilterIcon(field) {
  ...
  if (field === "source_system" || field === "provider_id" || field === "account_id" || field === "portfolio_id" || field === "liquidity") return "wallet";
...
function overviewGroupedInsightSections(cards = []) {
  const groups = [
    { id: "snapshot", title: "Snapshot", cards: [] },
    { id: "risk", title: "Alerts", cards: [] },
    { id: "capital", title: "Balance Sheet Signals", cards: [] },
    { id: "planning", title: "Planning Signals", cards: [] },
    { id: "data-quality", title: "Data Quality", cards: [] },
    { id: "trading", title: "Trading Signals", cards: [] },
...
function overviewMetricActionOptions(card = {}) {
  const text = overviewInsightCardSearchText(card);
  if (!text.trim()) return {};
  if (/(non[-\s]?liquid|non liquid capital)/.test(text)) {
    return metricActionOptions("filter-accounts", { "account-status": "active", liquidity: "non_liquid" }, `Show accounts for ${card.label || "metric"}`);
  }
  if (/(cash runway|liquid capital|liquidity|reserve target|idle liquidity)/.test(text)) {
    return metricActionOptions("filter-accounts", { "account-status": "active", liquidity: "liquid" }, `Show accounts for ${card.label || "metric"}`);
  }
  ...
  if (/investment share/.test(text)) {
    return metricActionOptions("filter-accounts", { "account-status": "active", "capital-bucket": "investment" }, `Show accounts for ${card.label || "metric"}`);
  }
...
function planningExitStrategyDashboard(portfolio = {}) {
  ...
  return `
    ...
    <section class="planning-target-grid exit-strategy-grid">
      ${panel("Roadmap", exitStrategyPhaseTable(phaseRows), "full")}
    </section>
...
function panel(title, body, variant = "", actions = "") {
  const icon = panelIcon(title);
  const isTablePanel = String(body || "").includes("minimal-table-wrap");
  const classes = ["panel", variant, isTablePanel ? "table-panel" : ""].filter(Boolean).join(" ");
  return `
    <section class="${safe(classes)}">
      <header class="panel-header">
        <h2>${icon ? `<span class="panel-title-icon" aria-hidden="true">${icons[icon]}</span>` : ""}<span>${safe(title)}</span></h2>
...
function dashboardToolbar(cards = [], title = "Metrics") {
  ...
  return metricLineDashboard([
    {
      ...
      html: metricLineItems([
        ...
        { label: "Liabilities", value: formatWholeCurrency(liabilities, "EUR"), meta: [`${formatPercent(percentOf(liabilities, netWorth))} of net worth`, "active liabilities"].join(" · "), note: "Credit and negative-balance exposure.", icon: "creditCard", options: metricActionOptions("filter-accounts", { "balance-sign": "negative", "account-status": "active" }, "Show liability accounts") },
...
function topAccountsMetricLines(rows = []) {
  return metricLineItems((rows || [])
...
function accountCreditMetricLines(rows = []) {
  return metricLineItems((rows || [])
    .filter((row) => numericValue(row.credit_limit_native) > 0)
    .slice(0, 6)
    .map((row) => {
  ...
  return metricLineDashboard([
    {
      ...
      html: metricLineItems([
        ...
        { label: "Net Flow", value: signedAmount(net, "EUR"), meta: percentOfIncomeNote(net, income, { signed: true, fallback: `${formatNumber(includedRows)} rows` }), note: "Income minus expenses in project currency.", icon: net >= 0 ? "trendUp" : "trendDown", options: metricActionOptions("filter-transactions", {}, "Show transactions for selected period") },
        { label: "Savings Rate", value: formatPercent(savingsRate), meta: periodLabel, note: "Net flow as a share of income.", icon: net >= 0 ? "trendUp" : "trendDown", options: metricActionOptions("filter-transactions", {}, "Show transactions for selected period") },
      ]),
    },
    {
      title: "Review Queue",
...
function transactionMetricPeriodLabel() {
  return state.period.mode === "all" ? "Full history" : periodValueLabel();
}

function planningTargetModelMetricSections(planning = {}) {
...
function transactionHeatmapMetric(label = "Activity", payload = {}, type = "expense") {
  const items = payload?.items || [];
  const activeItems = items.filter((row) => numericValue(row.amount_eur) > 0);
  const granularity = payload?.granularity || "day";
  const peak = activeItems.slice().sort((a, b) => numericValue(b.amount_eur) - numericValue(a.amount_eur))[0];
  const unit = granularity === "day" ? "days" : "periods";
  const peakLabel = peak ? heatmapRangeLabel(peak.date, granularity) : "";
  return {
    label,
    value: formatCurrency(payload?.total_eur || 0, "EUR"),
...
function transactionCurrencyFlowMetricLines(rows = []) {
  const total = rows.reduce((sum, row) => sum + Math.abs(numericValue(row.net_eur)), 0);
  return metricLineItems((rows || []).slice(0, 6).map((row) => {
    const net = numericValue(row.net_eur);
    return {
      label: row.currency || "-",
      value: signedWholeAmount(net, "EUR"),
      meta: `${formatPercent(percentOf(Math.abs(net), total))} of currency movement · ${signedWholeAmount(row.net_native || 0, row.currency || "EUR")} native · ${formatPlural(row.count || 0, "row")}`,
      note: `${formatWholeCurrency(row.income_eur || 0, "EUR")} income · ${formatWholeCurrency(row.expense_eur || 0, "EUR")} expense.`,
      icon: "currency",
...
function monthlyTargetIncomeCategoryMetricLines(rows = []) {
  ...
  return metricLineItems(values.map((row) => ({
    ...
    meta: `${formatPercent(percentOf(row.target, totalTarget))} of income model · ${formatWholeCurrency(row.actual, "EUR")} actual income`,
    note: "Planning income baseline category.",
    icon: insightIconFor(row.category, "category"),
...
function monthlyTargetCategoryMetricLines(rows = []) {
  ...
  return metricLineItems(values.map((row) => ({
    label: taxonomyLabel(row.category),
    value: formatWholeCurrency(row.target, "EUR"),
    meta: `${formatPercent(percentOf(row.target, totalTarget))} of expense model · ${formatWholeCurrency(row.actual, "EUR")} actual spend`,
    note: "Planning expense ceiling category.",
    icon: insightIconFor(row.category, "category"),
...
function tradeReturnActiveExposureMetricLines(data = {}) {
  ...
  return metricLineItems(exposureRows.slice(0, 6).map((row) => {
    ...
    return {
      ...
      meta: [`${formatPercent(percentOf(row.marketValue, marketTotal))} of active market value`, row.providerId || "", displayPortfolioId(row.portfolioId), `${signedWholeAmount(row.unrealized, row.currency)} unrealized`, signedPercent(returnPct)].filter(Boolean).join(" · "),
      note: "Open-position market exposure.",
...
function tradePerformanceMetricLines(rows = [], key = "period") {
  ...
  return metricLineItems((rows || []).slice(0, 6).map((row) => {
    ...
    const label = key === "period" ? performancePeriodLabel(row[key]) : row[key] || "-";
    const detail = key === "period"
      ? [
        `${signedPercent(row.return_pct || 0)} realized return`,
        `${formatPlural(row.trades || 0, "closed trade")}`,
    ...
    return {
      ...
      meta: `${formatPercent(percentOf(Math.abs(pl), total))} of shown P/L movement · ${detail}`,
      note: "Trade profit and loss movement.",
...
function tradePositionWatchlistMetricLines(data = {}) {
  const tables = data?.insight_tables || {};
  const insights = data?.insights || data?.summary || {};
  ...
  const attentionRows = (tables.attention || []).slice(0, 2).map((row) => ({
    label: row.name || "-",
    value: formatNumber(row.trades || 0),
    meta: "attention signal",
    note: row.note || "Trade rows requiring attention.",
    icon: "target",
  }));
  const items = [...attentionRows];
  ...
  if (largestExposure) {
    ...
    items.push({
      ...
      meta: `${formatPercent(percentOf(value, marketTotal))} of active market value · ${tradePositionLabel(largestExposure)}`,
      note: "Largest open-position exposure.",
      icon: "pie",
      options: tradeRowMetricOptions(largestExposure, "Show largest exposure"),
    });
  }

  const returnRows = activeRows.filter((row) => Number.isFinite(numericValue(row.unrealized_pl_pct)));
  const worstReturn = returnRows.slice().sort((a, b) => numericValue(a.unrealized_pl_pct) - numericValue(b.unrealized_pl_pct))[0];
  const bestReturn = returnRows.slice().sort((a, b) => numericValue(b.unrealized_pl_pct) - numericValue(a.unrealized_pl_pct))[0];
  if (worstReturn) {
    items.push({
  ...
  if (insights.latest_price_as_of || insights.oldest_price_as_of) {
    ...
    items.push({
      ...
      meta: insights.latest_price_as_of ? `Latest ${formatDisplayDate(insights.latest_price_as_of)}` : "",
      note: [
