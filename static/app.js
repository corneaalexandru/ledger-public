...
function settingsTabs() {
  ...
  return `
    <nav class="transaction-tabs settings-tabs" aria-label="Settings sections">
      ${tabs.map((tab) => `
        <button
          class="${state.settingsView === tab.id ? "is-active" : ""}"
          data-action="settings-tab"
          data-settings-view="${safe(tab.id)}"
          type="button"
        >${safe(tab.label)}</button>
      `).join("")}
    </nav>
  ...
  return `
    <div class="settings-list">
      ${rows.length ? rows.map((row) => `
        <article>
          <div>
            <strong>${safe(labelize(row.name))}</strong>
            <span>${safe(row.purpose)}</span>
          </div>
          <div class="settings-list-values">
            <span>${formatNumber(row.rows)} rows</span>
            <span>${formatNumber(row.columns)} cols</span>
            <em>${safe(row.status || "Ready")}</em>
          </div>
        </article>
      `).join("") : emptyState("No source truth registers available.")}
    </div>
  `;
}

function settingsConnectionList(rows) {
  return `
    <div class="settings-list">
      ${rows.length ? rows.map((row) => `
        <article>
          ...
          <div class="settings-list-values">
            <em>${safe(row.state)}</em>
          </div>
        </article>
      `).join("") : emptyState("No connection items available.")}
    </div>
  `;
}

function settingsRows(rows, actions = "") {
      ...
      ${normalizedRows.map((row) => `
        <article>
          <div>
            ...
            <span>${safe(row.note || "")}</span>
          </div>
          <div class="settings-list-values">
            <em>${row.valueHtml || safe(row.value)}</em>
          </div>
        </article>
      `).join("")}
      ${actions ? `<footer class="settings-actions">${actions}</footer>` : ""}
    </div>
  `;
}

function settingsSyncActions() {
  const refreshingPrices = Boolean(state.tradePriceRefresh?.loading);
  return `
    <div class="settings-sync-actions" aria-live="polite">
      ${tradePriceRefreshNotice()}
...
function settingsThresholdControls() {
  ...
  return `
    <div class="settings-threshold-list">
      ${rows.map(([label, field, suffix, note]) => `
        <label>
          <span>
            <strong>${safe(label)}</strong>
            <em>${safe(note)}</em>
